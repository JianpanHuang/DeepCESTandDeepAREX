...
%% Load data
load(['Data', filesep, 'DeepAREX_TrainData.mat']);
...
%% Train the network
[net, tr] = train(net, zR1Input, pTarget,'useParallel','yes','useGPU','no'); 
% Set to 'yes' if using GPU. In this case, variables should be transformed
% using 'gpuArray' before being fed into network, for example zR1Input = gpuArray(zR1Input)

%% Predict
outputs15 = net(zR1Input(:,tr.testInd),'useParallel','yes','useGPU','no'); % 15% test data
...
figure, plotregression(gather(pTarget(1, tr.testInd)), gather(outputs15(1, :)), 'B_0',...
                       ...
                       gather(pTarget(3, tr.testInd)), gather(outputs15(3, :)), 'rNOE',...
                       gather(pTarget(4, tr.testInd)), gather(outputs15(4, :)), 'MT');
% Regression of all test data
figure, plotregression(gather(pTarget(1, :)), gather(outputsAll(1, :)), 'B_0',...
                       gather(pTarget(2, :)), gather(outputsAll(2, :)), '3.5 ppm',...
                       gather(pTarget(3, :)), gather(outputsAll(3, :)), 'rNOE',...
                       gather(pTarget(4, :)), gather(outputsAll(4, :)), 'MT');
...
save(['Networks',filesep,'AREXNet', layName, gamaStr,'_', epochStr,'_', vperfStr, '.mat'],'net'); 
